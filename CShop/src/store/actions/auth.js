    ...
    axios
      ...
      .then(response => {
        const expiryDate = new Date(
          new Date().getTime() + response.data.expiryDate
        );
...
export const changePassword = (id, newPass) => {
  return dispatch => {
    ...
    let url =
      "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyDlIksbOmIbctM2LrIvzxQJznoKcyIUEJI";
    // if(!isSignup){
    ///       url= 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDlIksbOmIbctM2LrIvzxQJznoKcyIUEJI'
    //
    // }
    axios
      ...
      .then(response => {
        ...
        localStorage.setItem("userId", response.data.userId);
        dispatch(
          changePasswordSuccess(response.data.accessToken, response.data.userId)
        );
        dispatch(
          checkAuthTimeOut(response.data.expiryDate - new Date().getTime())
        );
      })
      .catch(err => {
